...
export function AccountDashboard({ profile: initialProfile, addresses: initialAddresses, orders }: Props) {
  ...
  return (
    <main className="bg-white px-5 py-12 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col justify-between gap-6 border-b border-ink/10 pb-8 sm:flex-row sm:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-clay">Your Deigon</p>
            <h1 className="mt-2 font-bold text-5xl text-ink">Account</h1>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[0.75fr_1.25fr] ">
          <aside className="h-fit border border-ink/10 bg-white p-5 rounded-xl">
            ...
            <nav className="mt-4 grid gap-2 text-sm">
              <a
                href="#profile"
                className="border-l-2 border-clay px-3 py-2 text-ink"
              >
                Profile
              </a>
              <a
                href="#addresses"
                className="px-3 py-2 text-ink/65 hover:text-ink"
              >
                Addresses
              </a>
              <a
          ...
          <div className="space-y-8">
            <section
              id="profile"
              className="border border-ink/10 bg-white p-6 sm:p-8 rounded-xl"
            >
              ...
              >
                ...
                <label className="text-sm text-ink">
                  ...
                  <input
                    value={profile.email}
                    readOnly
                    className="mt-2 w-full rounded-xl border border-ink/10 bg-ink/5 px-4 py-3 text-ink/55"
                  />
                </label>
                <label className="text-sm text-ink">
                  Phone
                ...
                <div className="flex items-end">
                  <button className="rounded-xl bg-ink px-5 py-3 text-sm font-semibold text-white hover:bg-black">
                    Save profile
                  </button>
                </div>
              </form>
              {profileMessage ? (
                <p className="mt-4 text-sm text-forest">{profileMessage}</p>
              ) : null}
            </section>
            ...
            <section
              ...
              className="border border-ink/10 bg-white p-6 sm:p-8 rounded-xl"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-clay">Addresses</p>
              <h2 className="mt-2 text-2xl font-semibold text-ink">
                Delivery details
              </h2>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {addresses.map((address) => (
                  <article
                    key={address.id}
                    className="border border-ink/10 p-4 text-sm text-ink/70"
                  >
                    ...
                    <div className="mt-4 flex gap-4 text-xs font-semibold uppercase tracking-wide">
                      <button
                        onClick={() => {
              ...
              >
                {Object.keys(addressForm).map((field) => (
                  <label key={field} className="text-sm capitalize text-ink">
                    {field.replace(/([A-Z])/g, " $1")}
                    <input
                      required={!["addressLine2", "phone"].includes(field)}
                ...
                <div className="sm:col-span-2 flex gap-3">
                  <button className="rounded-xl bg-ink px-5 py-3 text-sm font-semibold text-white">
              ...
              {addressMessage ? (
                <p className="mt-4 text-sm text-forest">{addressMessage}</p>
              ) : null}
            </section>
            ...
            <section
              ...
              className="border border-ink/10 bg-white p-6 sm:p-8 rounded-xl"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-clay">Orders</p>
              <h2 className="mt-2 text-2xl font-semibold text-ink">
                Order history
              </h2>
              {orders.length === 0 ? (
                <p className="mt-6 border border-dashed border-ink/15 px-5 py-10 text-center text-sm text-ink/55">
            ...
            <section
              id="security"
              className="border border-ink/10 bg-white p-6 sm:p-8 rounded-4xl"
            >
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-clay">Account security</p>
              <h2 className="mt-2 text-2xl font-semibold text-ink">
                Change password
              </h2>
              <form
              ...
              {passwordMessage ? (
                <p className="mt-4 text-sm text-forest">{passwordMessage}</p>
              ) : null}
            </section>
          </div>
        </div>
